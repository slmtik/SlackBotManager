...
using SlackBotManager.API.Models.SlackMessageManager;
using SlackBotManager.API.Models.Surfaces;
using SlackBotManager.API.Services;
using System.Text.Json;
using System.Text.Json.Nodes;

namespace SlackBotManager.API.Strategies;

public class CreatePullRequestInteraction : ISlashCommand, IViewSubmissionInteraction, IViewClosedInteraction, IBlockActionsInteraction
{
    ...
    private readonly IEnumerable<OptionObject<PlainTextObject>> _branches = [new(new("develop"), "develop"), new(new("release"), "release")];
    private readonly IEnumerable<OptionObject<PlainTextObject>> _tags = [new(new("#usefull"), "#usefull"), new(new("#easy"), "#easy")];
    ...
    public CreatePullRequestInteraction()
    {
        ...
        BlockActionsBindings.Add(("sectionBlockWithButton", "manage_details"), ShowManageDetailsView);
        BlockActionsBindings.Add(("manage_status", "review"), UpdatePullRequestStatus);
        BlockActionsBindings.Add(("manage_status", "approve"), UpdatePullRequestStatus);
        BlockActionsBindings.Add(("manage_status", "merge"), FinishPullRequest);
        BlockActionsBindings.Add(("manage_status", "close"), FinishPullRequest);
    }

    public async Task ShowCreatePullRequestView(SlackClient slackClient, CommandRequest commandRequest)
    {
        var userInfo = await slackClient.UserInfo(commandRequest.UserId);

        string message = $"{userInfo.User.Profile.DisplayName} is creating a new pull request";
        var postMessage = await slackClient.ChatPostMessage(new(commandRequest.ChannelId, message));
        
        var newPullRequest = new PullRequest(commandRequest.ChannelId, postMessage.TimeStampId) { Branches = ["develop"], IssuesNumber = 1 };
        await slackClient.ViewOpen(commandRequest.TriggerId, JsonSerializer.Serialize(BuildCreatePullRequestModal(newPullRequest), _jsonSerializerOptions));
    }

    private ModalView BuildCreatePullRequestModal(PullRequest pullRequest)
    {
        ...
        blockList.Add(new InputBlock("Tags", new MultiStaticSelect(_tags) { ActionId = "tags_select" }) { BlockId = "tags", Optional = true });
        blockList.Add(new SectionBlock(new PlainTextObject("Click to set branches or change number of issues"))
        {
            Accessory = new Button("Manage details") { ActionId = "manage_details" },
            BlockId = "sectionBlockWithButton",
        });
        ...
        return new ModalView("Create Pull Request", blockList)
        {
            ...
            Submit = new("Submit"),
            PrivateMetadata = JsonSerializer.Serialize(pullRequest, _jsonSerializerOptions)
        };
    }

    private async Task SubmitCreatePullRequestView(SlackClient slackClient, ViewSubmissionPayload viewSubmissionPayload)
    {
        PullRequest pullRequest = JsonSerializer.Deserialize<PullRequest>(viewSubmissionPayload.View.PrivateMetadata, _jsonSerializerOptions)!;
        ...
        if (viewSubmissionPayload.View.State.Values["tags"]["tags_select"] is MultiStaticSelectState selectedTags
        ...
        List<IBlock> blocks =
        [
            new SectionBlock(new MarkdownTextObject($"<@{viewSubmissionPayload.User.Id}> *has created a new pull request* `{string.Join(" ", pullRequest.Branches)}`"))
            {
                BlockId = "sectionBlockOnlyMrkdwn"
            },
            new DividerBlock(),
            new SectionBlock(links.Select(l => new MarkdownTextObject(l))) { BlockId = "sectionBlockOnlyFields" },
            new ActionBlock(
            [
            ...
            ]) { BlockId = "manage_status" },
        ];

        if (tags.Any())
        {
            blocks.Add(new ContextBlock(tags.Select(t => new PlainTextObject(t))));
        }
    ...
    private Task SubmitManageDetailsView(SlackClient slackClient, ViewSubmissionPayload viewSubmissionPayload)
    {
        ...
        return slackClient.ViewUpdate(viewId, JsonSerializer.Serialize(viewToCreatePullRequest, _jsonSerializerOptions));
    }

    public Task CloseCreatePullRequestView(SlackClient slackClient, ViewClosedPayload viewClosedPayload)
    {
        PullRequest pullRequest = JsonSerializer.Deserialize<PullRequest>(viewClosedPayload.View.PrivateMetadata, _jsonSerializerOptions)!;
    ...
    private Task ShowManageDetailsView(SlackClient slackClient, BlockActionsPayload payload)
    {
        ...
        PullRequest pullRequest = JsonSerializer.Deserialize<PullRequest>(privateMetadata, _jsonSerializerOptions)!;

        return slackClient.ViewPush(triggerId, JsonSerializer.Serialize(BuildManageDetailsModal(pullRequest), _jsonSerializerOptions));
    }

    private ModalView BuildManageDetailsModal(PullRequest pullRequest)
    {
        List<IBlock> blockList =
        [
        ...
        return new ModalView("Manage Details", blockList)
        {
    ...
    private async Task UpdatePullRequestStatus(SlackClient slackClient, BlockActionsPayload payload)
    {
        ...
        await slackClient.ChatUpdateMessage(new(payload.Channel!.Id,
                                                ...
                                                payload.Message.Blocks) { Metadata = payload.Message.Metadata });
        ...
        else if (pullRequestStatus == PullRequestStatus.Approve)
        {
            var displayName = payload.Message.Metadata!.EventPayload["user_profiles"]
                                                       .Deserialize<Dictionary<string, Profile>>(_jsonSerializerOptions)![payload.User.Id].DisplayName;
    ...
    private async Task<bool> UpdateReviewers(SlackClient slackClient, Message message, PullRequestStatus pullRequestStatus, string userId)
    {
        ...
        messageBlocks.Remove(messageBlocks.Find(b => b is ContextBlock && b.BlockId == "reviewers_block")!);

        var userProfiles = message.Metadata!.EventPayload["user_profiles"].Deserialize<Dictionary<string, Profile>>(_jsonSerializerOptions) ?? [];

        if (!userProfiles.ContainsKey(userId))
            userProfiles[userId] = (await slackClient.UserInfo(userId)).User.Profile;

        var reviewersBlocks = new List<IElement>() { new PlainTextObject("Reviewing:") };
        foreach (var item in reviewing)
            reviewersBlocks.Add(new Image(userProfiles[userId].DisplayName, userProfiles[userId].Image_24));

        if (approved.Count > 0)
        {
            reviewersBlocks.Add(new PlainTextObject("Approved:"));
            foreach (var item in approved)
                reviewersBlocks.Add(new Image(userProfiles[userId].DisplayName, userProfiles[userId].Image_24));
        }

        var reviewersBlock = new ContextBlock(reviewersBlocks) { BlockId = "reviewers_block" };
    ...
    private async Task FinishPullRequest(SlackClient slackClient, BlockActionsPayload payload)
    {
        ...
        messageBlocks.RemoveAll(b => (b is ContextBlock && b.BlockId == "reviewers_block") || (b is ActionBlock && b.BlockId == "manage_status"));

        var branches = payload.Message.Metadata.EventPayload["branches"].Deserialize<IEnumerable<string>>() ?? [];
        var userProfiles = payload.Message.Metadata.EventPayload["user_profiles"].Deserialize<Dictionary<string, Profile>>(_jsonSerializerOptions) ?? [];
